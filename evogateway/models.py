...
class ParsedMessage:
    ...
    # def topic_base(self, *, topics: MqttConfig.TopicLayout) -> str:
    #     """
    #     Build MQTT topic base according to device zone and type.
    ...
    #     Or for zone-independent devices:
    #         <system>/<controllers|relays>/<device>/<code>

    #     Where <device> = friendly alias in snake_case.
    #     """

    #     # Zone-independent devices
    #     RELAY_DEVICE_TYPES = {
    ...
    #     # Load topic settings (defensive: fall back to legacy defaults if fields missing)
    #     if topics is not None:
    #         zone_independent_topic = getattr(topics, "system", "_zone_independent")
    #         unknown_zone_topic     = getattr(topics, "zone_unknown", "_zone_unknown")
    #         zones_root_topic       = getattr(topics, "zones", "")
    #         relays_root_topic      = getattr(topics, "relays", "")
    #         controllers_root_topic = getattr(topics, "controller", "")
    #         dhw_root_topic         = getattr(topics, "dhw", "_dhw")
    #         dhw_is_zone            = getattr(topics, "dhw_is_zone", True)
    #         use_legacy             = getattr(topics, "use_legacy", True)
    #     else:
    #         # Legacy fallback if no topics object is passed
    #         zone_independent_topic = "_zone_independent"
    #         unknown_zone_topic     = "_zone_unknown"
    #         zones_root_topic       = "zones"
    #         relays_root_topic      = ""
    #         controllers_root_topic = ""
    #         dhw_root_topic         = "_dhw"
    #         dhw_is_zone            = True
    #         use_legacy             = True
    ...
    #     # Let's first identify if it is a dhw message 
    #     is_dhw = (
    #         self.zone_id == "HW"
    #         or (self.code and "dhw" in self.code.lower())
    #     )

    #     # Determine the "top" part before we wrap it into /zones/...
    #     if is_dhw and dhw_is_zone:
    #         # Treat DHW as just another zone 
    #         top = to_snake_case(dhw_root_topic).lower()

    #     elif is_dhw and not dhw_is_zone:
    #         # DHW is not treated as a zone; it gets its own top level bucket
    #         top = dhw_root_topic
    ...
    #     elif self.device_type == "01" or self.device_type == "18":
    #         # Controller or HGI
    #         top = (
    #             f"{zone_independent_topic}/{controllers_root_topic}"
    #             if controllers_root_topic else zone_independent_topic
    #         )
    ...
    #         or self.zone_id in RELAY_ZONE_IDS
    #     ):
    #         # Relays / UFH / OTB / zone-less devices
    #         top = (
    #             f"{zone_independent_topic}/{relays_root_topic}"
    #             if relays_root_topic else zone_independent_topic
    #         )
    ...
    #     # Message code (including evohome “domain” pseudo-zones & OTB msg_name)
    #     if self.zone_id in RELAY_ZONE_IDS:
    #         code = f"{self.code.lower()}/_domain_{self.zone_id.upper()}_{RELAY_ZONE_IDS[self.zone_id].lower()}"
    #     elif dev_type == "10" and "msg_name" in self.payload:
    #         code = f"{self.code.lower()}/{to_snake_case(self.payload['msg_name'])}"
    ...
    #     # New structure: wrap zones under /zones/<zone_name>/...
    #     # (including DHW-as-zone)
    #     if not use_legacy:
    #         # DHW-as-zone
    #         if is_dhw and dhw_is_zone:
    #             zone_name_snake = dhw_root_topic
    #             top = f"{zones_root_topic}/{zone_name_snake}"
    #         # Normal zones: only if we have a zone_id and a name
    #         elif self.zone_id and self.zone_name:
    #             zone_name_snake = to_snake_case(self.zone_name).lower()
    #             top = f"{zones_root_topic}/{zone_name_snake}"
